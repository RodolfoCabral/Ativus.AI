...
Versão final com correção do método execute
"""
...
try:
    ...
    with app.app_context():
        ...
        if not coluna_descricao:
            ...
            # Usar um valor padrão se nenhuma coluna for encontrada
            coluna_descricao = 'id'
            logger.warning(f"⚠️ Usando coluna ID como fallback: {coluna_descricao}")
        ...
        try:
            arquivo_path = 'routes/pmp_analytics.py'
            if os.path.exists(arquivo_path):
                with open(arquivo_path, 'r') as f:
                    conteudo = f.read()
