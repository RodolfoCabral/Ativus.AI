#!/usr/bin/env python3
"""
Script DEFINITIVO para forçar geração de OS preventivas
Usa valores padrão para campos obrigatórios sem depender de estruturas específicas
Execute com: heroku run python forcar_geracao_os_definitivo.py -a ativusai
"""
...
def forcar_geracao_os_definitivo():
    """Versão definitiva que funciona com qualquer estrutura"""
    
    print("🚀 GERAÇÃO DE OS PREVENTIVAS - VERSÃO DEFINITIVA")
    print("=" * 55)
        ...
        with app.app_context():
            ...
            for pmp_row in pmps_para_gerar:
                ...
                try:
                    ...
                    # Buscar dados básicos do equipamento (sem oficina)
                    ...
                    # Buscar filial do setor (se existir)
                    try:
                    ...
                    # Criar OS com valores padrão para campos obrigatórios
                    db.session.execute(db.text("""
                        INSERT INTO ordens_servico (
                            pmp_id, descricao, tipo_manutencao, prioridade, status,
                            equipamento_id, filial_id, setor_id, oficina,
                            qtd_pessoas, horas, hh, data_programada, data_criacao,
                    ...
                    """), {
                        ...
                        'filial_id': filial_id,
                        'setor_id': setor_id,
                        'oficina': 'Oficina Manutenção',  # Valor padrão fixo
                        'qtd_pessoas': int(num_pessoas),
                        'horas': float(tempo_pessoa),
                        'hh': float(tempo_pessoa) * int(num_pessoas),  # Horas-homem
                        ...
                        'condicao_ativo': 'Operacional'  # Valor padrão
                    })
            ...
            if os_geradas > 0:
                print(f"\n🎯 {os_geradas} OS preventivas foram criadas com sucesso!")
                ...
                try:
                    result_os = db.session.execute(db.text("""
                        SELECT id, descricao, status, data_programada, usuario_responsavel
                        FROM ordens_servico 
                    ...
                    for os_row in result_os:
                        responsavel = os_row[4] if os_row[4] else 'Sem responsável'
                        print(f"   - OS {os_row[0]}: {os_row[1][:40]}...")
                        print(f"     Status: {os_row[2]} | Data: {os_row[3]} | Responsável: {responsavel}")
                        
                except Exception as e:
                    print(f"   ⚠️ Não foi possível listar OS criadas: {e}")
